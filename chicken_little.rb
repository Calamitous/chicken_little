...
SMTP_SPLAT = [CONFIG[:smtp_server], CONFIG[:port], CONFIG[:domain], CONFIG[:account], CONFIG[:password], CONFIG[:authtype]]

def squawk(url, response)
  body = "Subject: [#{url.to_s}] is falling!\n"
  body += "OH NOES!!!\n  I expected a 200 from #{url.to_s} but got a #{response.to_s} instead!"

  Net::SMTP.start(*SMTP_SPLAT) { |sender| sender.sendmail(body, CONFIG[:sender], CONFIG[:recipients]) }
end
...
CONFIG[:urls_to_check].each do |url|
  begin
    ...
    squawk(url, response_code) unless response_code == '200'
  end
end
